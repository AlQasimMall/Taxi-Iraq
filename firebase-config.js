(function() {
    ...
    window.messaging.usePublicVapidKey('BI9cpoewcZa1ftyZ_bGjO0GYa4_cT0HNja4YFd6FwLwHg5c0gQ5iSj_MJZRhMxKdgJ0-d-_rEXcpSQ_cx7GqCSc');
    ...
    async function updateNotificationToken(userId) {
        try {
            ...
            if (permission === 'granted') {
                ...
                await window.db.ref(`users/${userId}/notificationToken`).set(token);
    ...
    window.messaging.onTokenRefresh(async () => {
        try {
            const token = await window.messaging.getToken();
